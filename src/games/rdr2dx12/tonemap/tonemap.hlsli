#ifndef SRC_GAMES_RDR2DX12_TONEMAP_TONEMAP_HLSLI_
#define SRC_GAMES_RDR2DX12_TONEMAP_TONEMAP_HLSLI_

// HLSL (DX12) port of reference/rdr2vk/tonemap/tonemap.glsl.
// Omitted the missing ../canvas.glsl include: this source uses no canvas symbols.
// GLSL fma is mapped to HLSL mad; floating-point fusion is not guaranteed.
// Boolean mix uses common.hlsli's RDR2Select(a, b, condition), selecting b
// independently for each true component, and a for each false component.
// Injection macros are supplied by the parent integration through shared.h.
...
float3 InverseReinhardScalablePiecewise(float3 color, float channel_max, float shoulder) {
  ...
  float3 denominator = exposure * ((float3)(channel_max) - color);
  float3 inversed = DivideSafe(numerator, denominator, (float3)(65504.0));

  return lerp(color, inversed, step((float3)(shoulder), color));
}
...
float3 Neutwo(float3 x, float peak) {
  ...
  float3 denominator_squared = mad(x, x, (float3)(p * p));
  return numerator * rsqrt(denominator_squared);
}

float3 Neutwo(float3 x, float3 peak) {
  float3 numerator = peak * x;
  float3 denominator_squared = mad(x, x, peak * peak);
  return numerator * rsqrt(denominator_squared);
}
...
float3 Neutwo_Inverse(float3 x, float peak, float clip) {
  float p = peak;
  float c = clip;
  float cc = c * c;
  float pp = p * p;
  ...
  float3 denominator_squared = mad(-xx, (float3)(cc - pp), (float3)(cc * pp));
...
float3 ApplyAnchoredAdaptationContrast(
  ...
  float3 flare_ratio = (float3)(1.0) + DivideSafe((float3)(flare), normalized + flare, (float3)(0.0));
  float3 exponent = contrast * flare_ratio;

  float3 ax_n = pow(ax, exponent);
  float3 anchor_n = pow(anchor_in, exponent);
  float3 response_target = ax_n / (ax_n + anchor_n);
  float3 response_baseline = ax / (ax + anchor_in);
  float3 gain = DivideSafe(response_target, response_baseline, (float3)(0.0));

  float3 contrasted_normalized = ax * gain / anchor_in;

  if (highlights != 1.0) {
    float3 highlight_distance = max(contrasted_normalized - 1.0, (float3)(0.0));
    contrasted_normalized += highlight_distance * (pow((float3)(1.0) + highlight_distance * highlight_distance, (float3)((highlights - 1.0) / 2.0)) - 1.0);
  }

  if (shadows != 1.0) {
    float3 shadow_distance = max((float3)(1.0) - contrasted_normalized, (float3)(0.0));
    contrasted_normalized *= pow((float3)(1.0) + shadow_distance * shadow_distance * shadow_distance, (float3)(shadows - 1.0));
  }
...
float ApplyAnchoredCInfinityBoundedPowerContrast(
  ...
  if (flare > 0.0) {
    ...
    exponent *= mad(flat_shadow_weight, flare / (normalized + flare), 1.0);
  }
  ...
  float bounded_displacement = contrast_displacement / mad(displacement_magnitude, exp2(-1.0 / displacement_magnitude), 1.0);
  float output_stops = mad(exponent, min(input_stops, 0.0), highlight_stops + bounded_displacement);
...
float3 ApplyAnchoredCInfinityBoundedPowerContrast(
  ...
  float3 ax = abs(color);
  float3 normalized = ax / anchor_in;
  float3 exponent = (float3)(contrast);

  if (flare > 0.0) {
    float3 shadow_distance = clamp((float3)(1.0) - normalized, (float3)(0.0), (float3)(1.0));
    float3 flat_shadow_weight = exp2(-normalized / shadow_distance);
    exponent *= mad(flat_shadow_weight, (float3)(flare) / (normalized + flare), (float3)(1.0));
  }

  float3 input_stops = log2(normalized);
  float3 highlight_stops = max(input_stops, (float3)(0.0));
  float3 contrast_displacement = (contrast - 1.0) * highlight_stops;
  float3 displacement_magnitude = abs(contrast_displacement);
  float3 bounded_displacement = contrast_displacement / mad(displacement_magnitude, exp2(-(float3)(1.0) / displacement_magnitude), (float3)(1.0));
  float3 output_stops = mad(exponent, min(input_stops, (float3)(0.0)), highlight_stops + bounded_displacement);
  float3 contrasted_normalized = exp2(output_stops);

  if (highlights != 1.0) {
    float3 highlight_distance = max(contrasted_normalized - 1.0, (float3)(0.0));
    float3 highlight_distance_squared = highlight_distance * highlight_distance;
    float3 flat_highlight_distance = ((float3)(1.0) + highlight_distance_squared) * exp2(-(float3)(1.0) / highlight_distance_squared);
    contrasted_normalized += highlight_distance * (pow((float3)(1.0) + flat_highlight_distance, (float3)((highlights - 1.0) / 2.0)) - 1.0);
  }

  if (shadows != 1.0) {
    float3 shadow_distance = clamp((float3)(1.0) - contrasted_normalized, (float3)(0.0), (float3)(1.0));
    float3 shadow_distance_squared = shadow_distance * shadow_distance;
    float3 flat_shadow_distance = shadow_distance_squared * shadow_distance * exp2((float3)(1.0) - (float3)(1.0) / shadow_distance_squared);
    contrasted_normalized *= pow((float3)(1.0) + flat_shadow_distance, (float3)(shadows - 1.0));
  }
...
float ApplyAnchoredCInfinityShoulder(float color, float peak, float anchor, float compression_strength) {
  ...
  float response_denominator = mad(distance_from_anchor, flat_weight, shoulder_range);
  return mad(shoulder_range, distance_from_anchor / response_denominator, color - distance_from_anchor);
}

float3 ApplyAnchoredCInfinityShoulder(float3 color, float3 peak, float3 anchor, float compression_strength) {
  float3 shoulder_range = peak - anchor;
  float3 distance_from_anchor = max(color - anchor, (float3)(0.0));
...
float3 ApplyToneMap(float3 _676, bool _679, float _638, float _m6, uint _m4, float _m10, float3 _488, float4 _489) {
  float3 untonemapped = _676;
  float3 tonemapped;
  if (RENODX_TONE_MAP_TYPE == 0.f) {
    float3 _742;
    if (_679) {
      float3 _719 = max((float3)(0.0), _676 * (_638 / _m6));
      float3 _722 = _719 * _488.x;
      _742 = (((((_719 * (_722 + (float3)(_489.x))) + (float3)(_489.y)) / ((_719 * (_722 + (float3)(_488.y))) + (float3)(_489.z))) - (float3)(_489.w)) * ((_m4 != 0u) ? _m10 : _488.z)) * _m6;
    } else {  // (x * (A * x + C) + D)/(x * (A * x + B) + E) - F
      float3 _685 = max((float3)(0.0), _676 * _638);
      float3 _688 = _685 * _488.x;
      _742 = clamp(((((_685 * (_688 + (float3)(_489.x))) + (float3)(_489.y)) / ((_685 * (_688 + (float3)(_488.y))) + (float3)(_489.z))) - (float3)(_489.w)) * _488.z, (float3)(0.0), (float3)(1.0));
    }
    tonemapped = _742;
    ...
    untonemapped = max((float3)(0.0), untonemapped * _638);
    if (RENODX_TONE_MAP_TYPE == 1.f) {  // Enhanced
      float3 bt709_white_lms = renodx_tonemap_psycho22_StockmanLMSFromBT709((float3)(1.0));
      ...
      float3 anchor_out_lms = renodx_tonemap_psycho22_StockmanLMSFromBT709((float3)(anchor_out));
      float pivot_slope = rdr2_tonemap_Derivative(pivot_point, A, B, C, D, E, F) * white_precompute;
      tonemapped = ApplyAnchoredAdaptationContrast(untonemapped, (2.0 * pivot_slope * pivot_point / anchor_out - 1.0) * 1.22, renodx_tonemap_psycho22_StockmanLMSFromBT709((float3)(pivot_point)), anchor_out_lms, 0.10f * pow(0.72f, 10.f), 1.f, 1.f);
      ...
      float3 precompression_lms = tonemapped;
      float precompression_yf = renodx_color_yf_from_LMS(precompression_lms);
      float3 peak_white_lms = renodx_tonemap_psycho22_StockmanLMSFromBT709((float3)(RENODX_PEAK_WHITE_NITS / RENODX_DIFFUSE_WHITE_NITS));
      float3 compressed_lms = ApplyAnchoredCInfinityShoulder(precompression_lms, peak_white_lms, anchor_out_lms, 2.f);

      tonemapped = renodx_tonemap_psycho23_ApplySignedOpponentRetentionAndGamutCompressionLMS(
          precompression_lms,
          compressed_lms,
          renodx_tonemap_psycho22_StockmanLMSFromBT709((float3)(pivot_point)),
          renodx_tonemap_psycho22_StockmanLMSFromBT709((float3)(anchor_out)),
          peak_white_lms,
          1.0,
          1.0);
      ...
      float3 base = rdr2_tonemap_Apply(untonemapped, A, B, C, D, E, F, white_precompute);
      tonemapped = rdr2_tonemap_ApplyExtended(untonemapped, base, pivot_point, white_precompute, A, B, C, D, E, F);
      tonemapped = lerp(tonemapped, base, RENODX_TONE_MAP_BLEND_STRENGTH);

      float uncompressed_yf = renodx_color_yf_from_BT709(tonemapped);

      tonemapped = ApplyAnchoredCInfinityShoulder(tonemapped, (float3)(RENODX_PEAK_WHITE_NITS / RENODX_DIFFUSE_WHITE_NITS), (float3)(rdr2_tonemap_Apply(pivot_point, A, B, C, D, E, F, white_precompute)), 2.f);
    ...
    float3 _685 = max((float3)(0.0), _676 * _638);
    float3 _688 = _685 * _488.x;
    tonemapped = clamp(((((_685 * (_688 + (float3)(_489.x))) + (float3)(_489.y)) / ((_685 * (_688 + (float3)(_488.y))) + (float3)(_489.z))) - (float3)(_489.w)) * _488.z, (float3)(0.0), (float3)(1.0));
  }

  return tonemapped;
}

float3 EncodeLUTInput(float3 x, float _m11, float _m12, float _m13, float _m14, bool skip_encoding) {
  if (CUSTOM_LUT_ENCODING != 0.f) {  // sRGB-like encoding (defaults to 2.2, controlled by the SDR gamma slider)
    ...
    float3 encoded = RDR2Select(
        RDR2Select(
            (pow(x, (float3)(_m12)) * _m13) - (float3)(_m14),
            x * _m11,
            (x < (float3)(0.0031308))),
        x, (bool3)skip_encoding);

    return encoded;
  }
}

float3 EncodeLUTInput(float3 x) {
  return (CUSTOM_LUT_ENCODING != 0.f) ? EncodeRDR2Gamma(x) : x;
}

float3 DecodeLUTInput(float3 lut_output, float3 lut_input, float compression_scale) {
  if (CUSTOM_LUT_ENCODING != 0.f) {  // Custom encoded domain
    ...
    if (CUSTOM_LUT_STRENGTH != 1.f) {
      lut_output = lerp(DecodeSRGB(lut_input), lut_output, CUSTOM_LUT_STRENGTH);
    }
    // if (RENODX_TONE_MAP_TYPE == 1.f) {
    //   lut_output = DecodeRDR2Gamma(EncodeSRGB(lut_output));
    // }
  } else {
    lut_output = lerp(lut_input, lut_output / compression_scale, CUSTOM_LUT_STRENGTH);
  }
  return lut_output;
}

float3 DecodeLUTInput(float3 lut_output, float3 lut_input) {
...
float CompressLUTInput(float3 color, bool use_encoding, uint _m5, float _m7, float _m8, float _m9) {
...
float3 CompressLUTInput(float3 color, bool use_encoding, uint _m5, float _m7, float _m8, float _m9, out float compression_scale) {
  bool use_custom_encoding = CUSTOM_LUT_ENCODING != 0.f;
  float3 compression_input = use_custom_encoding ? DecodeSRGB(color) : color;
...
float3 CompressLUTInputAlt(float3 color, uint _m3, float _m7, float _m8, float _m9, out float compression_scale) {
  bool use_custom_encoding = CUSTOM_LUT_ENCODING != 0.f;
  float3 compression_input = use_custom_encoding ? DecodeSRGB(color) : color;
...
float3 ApplyGradingAndDisplayMap(float3 ungraded_bt709, float2 texcoord) {
  if (IS_TONEMAPPED == 0.f) {
    ungraded_bt709 = GammaSafe(ungraded_bt709, false);

    float3 ungraded_bt2020 = BT2020FromBT709(ungraded_bt709);
    float3 graded_bt2020;
    if (RENODX_TONE_MAP_TYPE != 0.f && RENODX_TONE_MAP_TYPE != 3.f) {
      ...
      graded_bt2020 = max((float3)(0.0), graded_bt2020);
    ...
    float3 graded_bt709 = BT709FromBT2020(graded_bt2020);

    if (RENODX_TONE_MAP_TYPE != 0.f) {
      graded_bt709 *= (float3)(RENODX_DIFFUSE_WHITE_NITS / RENODX_GRAPHICS_WHITE_NITS);
    }
...
#endif  // SRC_GAMES_RDR2DX12_TONEMAP_TONEMAP_HLSLI_
