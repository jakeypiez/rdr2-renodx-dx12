#ifndef SRC_GAMES_RDR2DX12_INCLUDE_FILMGRAIN_HLSLI_
#define SRC_GAMES_RDR2DX12_INCLUDE_FILMGRAIN_HLSLI_

float GetLuminanceBT2020(float3 color) {
  return dot(color, float3(0.2627002120, 0.6779980715, 0.0593017165));
}
...
float RandomGenerate(float2 uv) {
  return Generate(uv);
}
...
float3 ApplyFilmGrainBT2020(float3 color_bt2020, float2 xy, float seed, float strength, float reference_white, bool debug) {
  ...
  color_bt2020 = max((float3)0.0, color_bt2020);
  ...
  float3 output_color = color_bt2020 * (1.0 + y_change);

  if (debug) {
    // Output visualization
    output_color = (float3)abs(y_change);
  }

  return output_color;
}

float3 ApplyFilmGrainBT2020(float3 color, float2 xy, float seed, float strength, float reference_white) {
  return ApplyFilmGrainBT2020(color, xy, seed, strength, reference_white, false);
}

float3 ApplyFilmGrainBT2020(float3 color, float2 xy, float seed, float strength) {
  return ApplyFilmGrainBT2020(color, xy, seed, strength, 1.0, false);
}

#endif  // SRC_GAMES_RDR2DX12_INCLUDE_FILMGRAIN_HLSLI_
