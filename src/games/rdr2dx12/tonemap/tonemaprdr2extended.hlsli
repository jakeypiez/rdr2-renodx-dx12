#ifndef RDR2DX12_TONEMAP_RDR2EXTENDED_HLSLI_
#define RDR2DX12_TONEMAP_RDR2EXTENDED_HLSLI_
...
float3 rdr2_tonemap_SolveCubic(float a3, float a2, float a1, float a0) {
...
float rdr2_tonemap_FindSecondDerivativeRootMax(float A, float B, float C, float D, float E) {
  ...
  float3 roots = rdr2_tonemap_SolveCubic(a3, a2, a1, a0);
...
float3 rdr2_tonemap_Apply(float3 x, float A, float B, float C, float D, float E, float F, float white_precompute) {
  // (x * (a * x + c) + d)/(x * (a * x + b) + e) - f
  float3 num = x * (A * x + C) + D;
  float3 den = x * (A * x + B) + E;
  return white_precompute * ((num / den) - F);
}
...
float3 rdr2_tonemap_ApplyExtended(
    float3 x,
    float3 base,
    float pivot_point,
    float white_precompute,
  ...
  float3 extended = slope * x + (float3)offset;  // match slope
  return lerp(base, extended, step(pivot_x, x));
}

#endif  // RDR2DX12_TONEMAP_RDR2EXTENDED_HLSLI_
